...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    // --- Generate PDF using serverless-compatible Puppeteer ---
    console.log("[oer/route.ts] Initializing Puppeteer for serverless environment");
    ...
    try {
      ...
      console.log("[oer/route.ts] Browser launched successfully");

      const page = await browser.newPage();
      console.log("[oer/route.ts] Setting HTML content...");
      
      await page.setContent(fullHtml, {
        waitUntil: 'load', // Changed from 'domcontentloaded' for better performance
        timeout: 30000,
      });
      ...
      console.error('[oer/route.ts] Puppeteer error:', puppeteerError);
